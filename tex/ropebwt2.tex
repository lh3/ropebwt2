...
and and the only tool that practically works for reads of averaged kilobases in
length.
...
{\it Venter}: 32M$\times$875bp (in average) human reads with Sanger sequencing (\citealt*{Levy:2007uq}; http://bit.ly/levy2007);
{\it mol}: 23M$\times$4026bp (in average) human reads with Illumina's Moleculo sequencing (http://bit.ly/mol12878).
...
Table~1 shows that for short reads, ropeBWT2 has comparable performance to
...
strings from a BWT and to generate a sampled suffix array in parallel,
though these functionalities have not been implemented.
\vspace*{-1em}
