...
\Begin {
	...
	$c\gets \$$\;
	\For{$i\gets|P|-1$ \KwTo $-1$} {
		$[l,u)\gets${\sc InsertAux}$(B,P[i],l,u,P[i+1],{\it is\_RCLO})$\;
	}
	{\bf return} $B$
}
\textbf{Function} {\sc InsertAux}$(B,c',l,u,c,{\it is\_RCLO})$
\Begin {
	$k\gets l$\;
...
\Begin {
	...
	\While{$|A|\not=0$} {
		...
		\nl\For{$0\le j<|A|$} {
			...
			$[A(j).l,A(j).u)\gets${\sc InsertAux}$(B,c,A(j).l,A(j).u,A(j).c,{\it is\_RCLO})$\;
		}
